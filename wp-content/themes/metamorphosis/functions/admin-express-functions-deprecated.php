...
function express_prime_taxonomy($args) {
	...
	// Re-assign the taxonomies so they are compatible with primeThemes themes
			...
			if ($taxonomy['taxonomy'] == 'tumblog') {
				foreach ($taxonomy['tags'] as $tag) {
					switch (strtolower($tag)) {
						case 'note':
							$prime_tags[] = get_option('prime_articles_term_id');
							break;
						case 'link':
							$prime_tags[] = get_option('prime_links_term_id');
							break;
						case 'quote':
							$prime_tags[] = get_option('prime_quotes_term_id');
							break;
						case 'image':
							$prime_tags[] = get_option('prime_images_term_id');
							break;
						default:
							$prime_tags[] = $tag;
							break;
				...
				$taxonomy['tags'] = implode(',', $prime_tags);
			}
...
function express_newPost($args) {
	...
	$args = express_prime_taxonomy($args);
