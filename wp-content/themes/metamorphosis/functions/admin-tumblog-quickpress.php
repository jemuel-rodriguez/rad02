...
if ($content_method == 'post_format') {
	...
	$iphone_function_file = TEMPLATEPATH . '/functions/' . 'admin-express-functions-deprecated.php';
}
...
if (current_user_can('publish_posts')) {
	//Register Actions
	add_action('wp_ajax_prime_tumblog_media_upload', 'prime_tumblog_file_upload');
	add_action('wp_ajax_prime_tumblog_post', 'prime_tumblog_ajax_post');
	add_action('wp_ajax_nopriv_prime_tumblog_post', 'prime_tumblog_ajax_post');
	//Load scripts and libraries
	add_action('admin_print_scripts-index.php', 'prime_load_tumblog_libraries');
	add_action('admin_print_styles', 'prime_load_tumblog_css',10,1);
	// Hook into the 'wp_dashboard_setup' action to register Tumblog Dashboard Widget
	add_action('wp_dashboard_setup', 'prime_register_tumblog_dashboard_widget' );
}

//Adds the Tumblog Dashboard Widget to the WP Dashboard
function prime_register_tumblog_dashboard_widget() {
	wp_add_dashboard_widget('prime_tumblog_dashboard_widget', 'Tumblog', 'prime_tumblog_dashboard_widget_output');	
	// Globalize the metaboxes array, this holds all the widgets for wp-admin
	global $wp_meta_boxes;
	// Get the regular dashboard widgets array 
	$normal_dashboard = $wp_meta_boxes['dashboard']['normal']['core'];
	// Backup and delete dashboard widget from the end of the array
	$prime_tumblog_widget_backup = array('prime_tumblog_dashboard_widget' => $normal_dashboard['prime_tumblog_dashboard_widget']);
	unset($normal_dashboard['prime_tumblog_dashboard_widget']);
	// Merge the two arrays together so tumblog widget is at the beginning
	$sorted_dashboard = array_merge($prime_tumblog_widget_backup, $normal_dashboard);
	// Save the sorted array back into the original metaboxes 
	$wp_meta_boxes['dashboard']['normal']['core'] = $sorted_dashboard;
} 

//Loads Tumblog javascript and php js functions
function prime_load_tumblog_libraries() {
...
//Load Tumblog CSS
function prime_load_tumblog_css($hook) {
...
function prime_tumblog_ajax_post() {
	...
	if ($_POST['tumblog-type'] == 'article') 
	{
		$data = $_POST;
		$type = 'note';
		prime_tumblog_publish($type, $data);
		die ( 'OK' );
	}
	//Publish Video
...
function prime_tumblog_publish($type, $data) {
	global $current_user;
	...
	//get term ids
	$tumblog_items = array(	'articles'	=> get_option('prime_articles_term_id'),
							'images' 	=> get_option('prime_images_term_id'),
							'audio' 	=> get_option('prime_audio_term_id'),
							'video' 	=> get_option('prime_video_term_id'),
							'quotes'	=> get_option('prime_quotes_term_id'),
							'links' 	=> get_option('prime_links_term_id')
							);
	//Set date formatting
...
//Handles AJAX Post
function prime_tumblog_file_upload() {
	global $wpdb; 
	//Upload overrides
...
function prime_tumblog_dashboard_widget_output() {
	...
	if (current_user_can('publish_posts')) {
		...
		jQuery(document).ready(function(){
			...
			jQuery('#articles-menu-button').click(function ()
			{
				...
				<?php if (get_option('prime_tumblog_content_method') != 'post_format') { ?>
			...
			new AjaxUpload('#image_upload_button', {
	  			...
	  			name: 'userfile',
	  			data: { // Additional data to send
							action: 'prime_tumblog_media_upload',
							type: 'upload',
							data: 'userfile' },
	...
	<div id="tumblog-post">
		...
		<div id="meta-fields" class="hide-fields">
			...
			<br />
			<div id="additional-categories" style="width:<?php if (get_option('prime_tumblog_content_method') != 'post_format') { ?>47%;float:left;<?php } else { ?>94%;<?php } ?>">
