function primeTabMaker(h, i, f) {
    ...
    this.buildSelectControl = function () {
        ...
        this.selectControl = jQuery("<select></select>").attr("id", "prime-tab-select").addClass(f ? f : "");
    ...
    this.buildTextInputControl = function ( id ) {
    	...
    	var labelElement = '<label for="prime_tab_title">Tab ' + id + ' Title</label>';
    	var inputElement = '<input type="text" id="prime_tab_title_' + id.toString() + '" class="txt input-text" name="prime_tab_title" />';
    ...
    this.buildTabButtons = function (a) {
        if (this.buttonsControl) {
			...
			jQuery('label[for="prime_tab_title"]').parents('tr').remove();
