...
function primethemes_add_admin() {
    ...
    $super_user = get_option('framework_prime_super_user');
    ...
    $shortname =  get_option('prime_shortname'); 
   	...
   	if ( isset( $_REQUEST['page'] ) ) {
		...
		if ( isset( $_REQUEST['prime_save'] ) ) {
		
			$_action = mysql_real_escape_string( strtolower( trim( strip_tags( $_REQUEST['prime_save'] ) ) ) );
		...
		if ( $_action == 'reset' ) {
			...
			if ( function_exists( 'check_ajax_referer' ) ) { check_ajax_referer( 'primeframework-theme-options-reset', '_ajax_nonce' ); } // End IF Statement
			...
			switch ( $_page ) {
				...
				case 'primethemes':
					...
					prime_reset_options($options,'primethemes');
					header("Location: admin.php?page=primethemes&reset=true");
					die;
					
				break;
				
				case 'primethemes_framework_settings':
				
					$options = get_option('prime_framework_template'); 
					prime_reset_options($options);
					header("Location: admin.php?page=primethemes_framework_settings&reset=true");
					die;
				
				break;
				
				case 'primethemes_seo':
				
					$options = get_option('prime_seo_template'); 
					prime_reset_options($options);
					header("Location: admin.php?page=primethemes_seo&reset=true");
					die;
				
				break;
				
				case 'primethemes_sbm':
				
					delete_option('sbm_prime_sbm_options');
					header("Location: admin.php?page=primethemes_sbm&reset=true");
					die;
	...
	if(get_option('framework_prime_backend_icon')) { $icon = get_option('framework_prime_backend_icon'); }
	else { $icon = get_bloginfo('template_url'). '/functions/images/prime-icon.png'; }
	
    if(function_exists('add_object_page'))
    {
        add_object_page ('Page Title', $themename, 'manage_options','primethemes', 'primethemes_options_page', $icon);
    }
    else
    {
        add_menu_page ('Page Title', $themename, 'manage_options','primethemes_home', 'primethemes_options_page', $icon); 
    }
    $primepage = add_submenu_page('primethemes', $themename, 'Theme Options', 'manage_options', 'primethemes','primethemes_options_page'); // Default
    
    // Framework Settings Menu Item
    $primeframeworksettings = '';
    if($super_user == $current_user_id || empty($super_user)) {
		$primeframeworksettings = add_submenu_page('primethemes', 'Framework Settings', 'Framework Settings', 'manage_options', 'primethemes_framework_settings', 'primethemes_framework_settings_page');
	}
			
	// Add SEO Menu Item
	$primeseo = '';
	if ( get_option('framework_prime_seo_disable') != 'true' ) 
		$primeseo = add_submenu_page('primethemes', 'SEO', 'SEO', 'manage_options', 'primethemes_seo', 'primethemes_seo_page');
	
	// Add Sidebar Manager Menu Item
	$primesbm = '';
	if ( get_option('framework_prime_sbm_disable') != 'true' ) 
		$primesbm = add_submenu_page('primethemes', 'Sidebar Manager', 'Sidebar Manager', 'manage_options', 'primethemes_sbm', 'primethemes_sbm_page');

	// primethemes Content Builder
	if (function_exists('primethemes_content_builder_menu')) {
		primethemes_content_builder_menu();
	}
	
	// Custom Navigation Menu Item
	if (function_exists('prime_custom_navigation_menu')) {
		prime_custom_navigation_menu();
	}	

	// Update Framework Menu Item
    if($super_user == $current_user_id || empty($super_user)) {
         $primethemepage = add_submenu_page('primethemes', 'primeFramework Update', 'Update Framework', 'manage_options', 'primethemes_framework_update', 'primethemes_framework_update_page');    
    }
	
	// Buy Themes Menu Item
    if(get_option('framework_prime_buy_themes_disable') != 'true') {
        $primethemepage = add_submenu_page('primethemes', 'Available primeThemes', 'Buy Themes', 'manage_options', 'primethemes_themes', 'primethemes_more_themes_page');    
    	add_action("admin_print_scripts-$primethemepage", 'prime_load_only' );
	}
	
	// Add framework functionaily to the head individually
	add_action("admin_print_scripts-$primepage", 'prime_load_only');
	add_action("admin_print_scripts-$primeframeworksettings", 'prime_load_only');
	add_action("admin_print_scripts-$primeseo", 'prime_load_only');
	add_action("admin_print_scripts-$primesbm", 'prime_load_only');
...
function prime_reset_options($options,$page = ''){
	...
	//When Theme Options page is reset - Add the prime_options option
	if($page == 'primethemes'){
		delete_option('prime_options');
	}
}
}
...
function primethemes_options_page(){
	...
	$update_message = '';
    if(get_option('framework_prime_theme_version_checker') == 'true') {
        $update_message = primethemes_version_checker($local_version);
    }
...
<div id="prime-popup-reset" class="prime-save-popup"><div class="prime-save-reset">Options Reset</div></div>
    ...
    <?php
    	...
    	if ( function_exists( 'wp_nonce_field' ) ) { wp_nonce_field( 'primeframework-theme-options-update' ); } // End IF Statement
    	
    	$prime_nonce = '';
    	
    	if ( function_exists( 'wp_create_nonce' ) ) { $prime_nonce = wp_create_nonce( 'primeframework-theme-options-update' ); } // End IF Statement
    ...
    ?>
        ...
        $return = primethemes_machine($options);
        ?>
        ...
        <?php
	    	// Add nonce for added security.
	    	if ( function_exists( 'wp_nonce_field' ) ) { wp_nonce_field( 'primeframework-theme-options-reset' ); } // End IF Statement
	    	
	    	$prime_nonce = '';
	    	
	    	if ( function_exists( 'wp_create_nonce' ) ) { $prime_nonce = wp_create_nonce( 'primeframework-theme-options-reset' ); } // End IF Statement
...
function prime_load_only() {
	...
	add_action('admin_head', 'prime_admin_head');
	...
	function prime_admin_head() { 
		...
		jQuery(document).ready(function(){
			...
			//JQUERY DATEPICKER
			jQuery('.prime-input-calendar').each(function (){
			...
			jQuery('.prime-input-time').each(function (){
				jQuery('#' + jQuery(this).attr('id')).mask("99:99");
			});
			
			//Color Picker
			<?php $options = get_option('prime_template');
		...
		<script type="text/javascript">
			...
			jQuery('#expand_options').click(function(){
				if(flip == 0){
					flip = 1;
					jQuery('#prime_container #prime-nav').hide();
					jQuery('#prime_container #content').width(785);
					jQuery('#prime_container .group').add('#prime_container .group h2').show();
	
					jQuery(this).text('[-]');
					
				} else {
					flip = 0;
					jQuery('#prime_container #prime-nav').show();
					jQuery('#prime_container #content').width(595);
					jQuery('#prime_container .group').add('#prime_container .group h2').hide();
					jQuery('#prime_container .group:first').show();
					jQuery('#prime_container #prime-nav li').removeClass('current');
					jQuery('#prime_container #prime-nav li:first').addClass('current');
				...
				jQuery('.prime-radio-img-img').click(function(){
					jQuery(this).parent().parent().find('.prime-radio-img-img').removeClass('prime-radio-img-selected');
					jQuery(this).addClass('prime-radio-img-selected');
					
				});
				jQuery('.prime-radio-img-label').hide();
				jQuery('.prime-radio-img-img').show();
				jQuery('.prime-radio-img-radio').hide();
				jQuery('#prime-nav li:first').addClass('current');
				jQuery('#prime-nav li a').click(function(evt){
				
						jQuery('#prime-nav li').removeClass('current');
				...
				jQuery('select.prime-typography-unit').change(function(){
					var val = jQuery(this).val();
					var parent = jQuery(this).parent();
					var name = parent.find('.prime-typography-size-px').attr('name');
					if(name == ''){ var name = parent.find('.prime-typography-size-em').attr('name'); } 
					
					if(val == 'px'){ 
						parent.find('.prime-typography-size-em').hide().removeAttr('name');
						parent.find('.prime-typography-size-px').show().attr('name',name);
					}
					else if(val == 'em'){
						parent.find('.prime-typography-size-em').show().attr('name',name);
						parent.find('.prime-typography-size-px').hide().removeAttr('name');
					}
				...
				if('<?php echo $is_reset; ?>' == 'true'){
					...
					var reset_popup = jQuery('#prime-popup-reset');
					reset_popup.fadeIn();
			...
			jQuery('#prime-popup-save').center();
			jQuery('#prime-popup-reset').center();
			...
			new AjaxUpload(clickedID, {
				  ...
				  onComplete: function(file, response) {
					...
					if(response.search('Upload Error') > -1){
						...
						var buildReturn = '<img class="hide prime-option-image" id="image_'+clickedID+'" src="'+response+'" alt="" />';
			...
			jQuery('.string_builder_add').click(function(){
					...
					var data = {
						...
						data: data, 
						_ajax_nonce: '<?php echo $prime_nonce; ?>'
					};
			...
			jQuery('#primeform').submit(function(){
				
					function newValues() {
					  var serializedValues = jQuery("#primeform *").not('.prime-ignore').serialize();
					...
					var data = {
						<?php if(isset($_REQUEST['page']) && $_REQUEST['page'] == 'primethemes'){ ?>
						type: 'options',
						<?php } ?>
						<?php if(isset($_REQUEST['page']) && $_REQUEST['page'] == 'primethemes_framework_settings'){ ?>
						type: 'framework',
						<?php } ?>
						<?php if(isset($_REQUEST['page']) && $_REQUEST['page'] == 'primethemes_seo'){ ?>
						type: 'seo',
						<?php } ?>
						<?php if(isset($_REQUEST['page']) && $_REQUEST['page'] == 'primethemes_tumblog'){ ?>
						type: 'tumblog',
						<?php } ?>
...
if (!function_exists('prime_ajax_callback')) {
function prime_ajax_callback() {
	
	// check security with nonce.
	if ( function_exists( 'check_ajax_referer' ) ) { check_ajax_referer( 'primeframework-theme-options-update', '_ajax_nonce' ); } // End IF Statement
	...
	elseif ($save_type == 'options' OR $save_type == 'seo' OR $save_type == 'tumblog' OR $save_type == 'framework') {
		...
		//Pull options
        	$options = get_option('prime_template');
		if($save_type == 'seo'){ 
			$options = get_option('prime_seo_template'); } // Use SEO template on SEO page
		if($save_type == 'tumblog'){ 
			$options = get_option('prime_tumblog_template'); } // Use Tumblog template on Tumblog page
		if($save_type == 'framework'){ 
			$options = get_option('prime_framework_template'); } // Use Framework template on Framework Settings page
		...
		foreach($options as $option_array){
			...
			if(isset($option_array['id'])) { // Non - Headings...
				...
				if($id == 'framework_prime_import_options'){
	...
	if( $save_type == 'options' OR $save_type == 'framework' ){
		...
		foreach ($data as $name => $value){
				...
				if(is_serialized($value)) {
					...
					$value = $temp_options;
					$prime_array[$name] = $prime_array_option;
				} else {
					$prime_array[$name] = $value;
				}
		...
		update_option('prime_settings_encode',$output);
...
function primethemes_machine($options) {
	...
	foreach ($options as $value) {
		...
		case 'text':
			...
			if ( $std != "") { $val = $std; }
			$output .= '<input class="prime-input" name="'. $value['id'] .'" id="'. $value['id'] .'" type="'. $value['type'] .'" value="'. $val .'" />';
		break;
		
		case 'select':

			$output .= '<div class="select_wrapper"><select class="prime-input" name="'. $value['id'] .'" id="'. $value['id'] .'">';
		...
		case 'calendar':
			...
			$std = get_option($value['id']);
			if ( $std != "") { $val = $std; }
            $output .= '<input class="prime-input-calendar" type="text" name="'.$value['id'].'" id="'.$value['id'].'" value="'.$val.'">';
		...
		case 'time':
			...
			$std = get_option($value['id']);
			if ( $std != "") { $val = $std; }
			$output .= '<input class="prime-input-time" name="'. $value['id'] .'" id="'. $value['id'] .'" type="text" value="'. $val .'" />';
		break;
		case 'textarea':
			...
			if(isset($value['std'])) {
				...
				$output .= '<textarea class="prime-input" name="'. $value['id'] .'" id="'. $value['id'] .'" cols="'. $cols .'" rows="8">'.$ta_value.'</textarea>';
		...
		case "radio":
			 ...
			 { 
				...
				$output .= '<input class="prime-input prime-radio" type="radio" name="'. $value['id'] .'" value="'. $key .'" '. $checked .' />' . $option .'<br />';
		...
		case "checkbox": 
		   ...
		   $checked = '';
			...
			$output .= '<input type="checkbox" class="checkbox prime-input" name="'.  $value['id'] .'" id="'. $value['id'] .'" value="true" '. $checked .' />';
		...
		case "multicheck":
			...
			$prime_key = $value['id'] . '_' . $key;
			$saved_std = get_option($prime_key);
			...
			elseif( $std == $key) {
			   $checked = 'checked="checked"';
				...
				$checked = '';                                                                                    }
			$output .= '<input type="checkbox" class="checkbox prime-input" name="'. $prime_key .'" id="'. $prime_key .'" value="true" '. $checked .' /><label for="'. $prime_key .'">'. $option .'</label><br />';
		...
		case "upload":
			...
			if ( function_exists( 'primethemes_medialibrary_uploader' ) ) {
				...
				$output .= primethemes_medialibrary_uploader( $value['id'], $value['std'], null ); // New AJAX Uploader using Media Library
			
			} else {
			
				$output .= primethemes_uploader_function($value['id'],$value['std'],null); // Original AJAX Uploader
		...
		case "upload_min":
			...
			if ( function_exists( 'primethemes_medialibrary_uploader' ) ) {
				...
				$output .= primethemes_medialibrary_uploader( $value['id'], $value['std'], 'min' ); // New AJAX Uploader using Media Library
			
			} else {
			
				$output .= primethemes_uploader_function($value['id'],$value['std'],'min'); // Original AJAX Uploader
				
			} // End IF Statement
			
			// $output .= primethemes_uploader_function($value['id'],$value['std'],'min');
		...
		case "color":
			...
			$output .= '<input class="prime-color" name="'. $value['id'] .'" id="'. $value['id'] .'" type="text" value="'. $val .'" />';
		break;   
		...
		case "typography":
			...
			$output .= '<select class="prime-typography prime-typography-size prime-typography-size-px"  id="'. $value['id'].'_size" '. $name_px . $show_px .'>';
				for ($i = 9; $i < 71; $i++){ 
					if($val == strval($i)){ $active = 'selected="selected"'; } else { $active = ''; }
					$output .= '<option value="'. $i .'" ' . $active . '>'. $i .'</option>'; }
			$output .= '</select>';
			$output .= '<select class="prime-typography prime-typography-size prime-typography-size-em" id="'. $value['id'].'_size" '. $name_em . $show_em.'>';
				$em = 0.5;
				for ($i = 0; $i < 39; $i++){
			...
			$output .= '<select class="prime-typography prime-typography-unit" name="'. $value['id'].'_unit" id="'. $value['id'].'_unit">';
			...
			$output .= '<select class="prime-typography prime-typography-face" name="'. $value['id'].'_face" id="'. $value['id'].'_face">';
			...
			// Custom Font stack
			$new_stacks = get_option('framework_prime_font_stack');
			if(!empty($new_stacks)){
			...
			$output .= '<select class="prime-typography prime-typography-style" name="'. $value['id'].'_style" id="'. $value['id'].'_style">';
			...
			$output .= '<input class="prime-color prime-typography prime-typography-color" name="'. $value['id'] .'_color" id="'. $value['id'] .'_color" type="text" value="'. $val .'" />';
		...
		case "border":
			...
			$output .= '<select class="prime-border prime-border-width" name="'. $value['id'].'_width" id="'. $value['id'].'_width">';
				for ($i = 0; $i < 21; $i++){ 
			...
			$output .= '<select class="prime-border prime-border-style" name="'. $value['id'].'_style" id="'. $value['id'].'_style">';
			...
			$output .= '<div id="' . $value['id'] . '_color_picker" class="colorSelector"><div></div></div>';
			$output .= '<input class="prime-color prime-border prime-border-color" name="'. $value['id'] .'_color" id="'. $value['id'] .'_color" type="text" value="'. $val .'" />';
		...
		case "images":
			...
			foreach ($value['options'] as $key => $option) 
			 ...
			 $i++;
				 ...
				 $selected = '';
				   if($select_value != '') {
						if ( $select_value == $key) { $checked = ' checked'; $selected = 'prime-radio-img-selected'; } 
				    } else {
						if ($value['std'] == $key) { $checked = ' checked'; $selected = 'prime-radio-img-selected'; }
						elseif ($i == 1  && !isset($select_value)) { $checked = ' checked'; $selected = 'prime-radio-img-selected'; }
						elseif ($i == 1  && $value['std'] == '') { $checked = ' checked'; $selected = 'prime-radio-img-selected'; }
						else { $checked = ''; }
					}	
				
				$output .= '<span>';
				$output .= '<input type="radio" id="prime-radio-img-' . $value['id'] . $i . '" class="checkbox prime-radio-img-radio" value="'.$key.'" name="'. $value['id'].'" '.$checked.' />';
				$output .= '<div class="prime-radio-img-label">'. $key .'</div>';
				$output .= '<img src="'.$option.'" alt="" class="prime-radio-img-img '. $selected .'" onClick="document.getElementById(\'prime-radio-img-'. $value['id'] . $i.'\').checked = true;" />';
				$output .= '</span>';
		...
		case "string_builder":
			...
			$output .= 'Name<input class="prime-input prime-ignore" name="name" id="'. $value['id'] .'_name" type="text" />';
			$output .= 'Font Stack<input class="prime-input prime-ignore" name="value" id="'. $value['id'] .'_value" type="text" />';
		...
		case "heading":
			...
			$jquery_click_hook = "prime-option-" . $jquery_click_hook;
//			$jquery_click_hook = "prime-option-" . str_replace("&","",str_replace("/","",str_replace(".","",str_replace(")","",str_replace("(","",str_replace(" ","",strtolower($value['name'])))))));
		...
		if ( is_array($value['type'])) {
			foreach($value['type'] as $array){
					...
					if($array['type'] == 'text') { // Only text at this point
						 ...
						 $output .= '<input class="input-text-small prime-input" name="'. $id .'" id="'. $id .'" type="text" value="'. $std .'" />';  
    ...
    if ( isset($_REQUEST['page']) && $_REQUEST['page'] != 'primethemes_content_builder' ) {
		$output .= '</div>';
	}
...
function primethemes_uploader_function($id,$std,$mod){
	...
	if($mod != 'min') { 
            ...
            $uploader .= '<input class="prime-input" name="'. $id .'" id="'. $id .'_upload" type="text" value="'. $val .'" />';
	}
	...
	if(!empty($upload)){
    	...
    	$uploader .= '<a class="prime-uploaded-image" href="'. $upload . '">';
    	$uploader .= '<img class="prime-option-image" id="image_'.$id.'" src="'.$upload.'" alt="" />';
    	$uploader .= '</a>';
		}
	$uploader .= '<div class="clear"></div>' . "\n"; 
...
function primethemes_version_checker ($local_version) {
	...
	$theme_name = str_replace("-","",strtolower(get_option('prime_themename')));
	$feed_url = 'http://www.primethemes.com/?feed=updates&theme=' . $theme_name;
	...
	if(isset($pieces_rss[0]) && $pieces_rss[0] != 0) {
		...
		if ($version_sentinel == true) {
			$update_message = '<div class="update_available">Theme update is available (v.' . $latest_version_via_rss . ') - <a href="http://www.primethemes.com/amember">Get the new version</a>.</div>';
		}
		else {
			$update_message = '';
		}
