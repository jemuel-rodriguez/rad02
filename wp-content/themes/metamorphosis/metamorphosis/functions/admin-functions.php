...
function prime_show_pagemenu( $exclude="" ) {
    ...
    if ( get_option('prime_ex_featpages') ) {
        $menupages = get_option('prime_featpages');
...
function primethemes_framework_update_page(){
        ...
        elseif(isset($_POST['prime_ftp_cred'])){
        ...
        $url = admin_url('admin.php?page=primethemes_framework_update');
        ?>
            ...
            <?php 
            $local_version = get_option('prime_framework_version');
            ...
            <form method="post"  enctype="multipart/form-data" id="primeform" action="<?php /* echo $url; */ ?>">
                
                <?php if( $upd ) { ?>
                <?php wp_nonce_field('update-options'); ?>
                <h3>A new version of primeFramework is available.</h3>
                <p>This updater will collect a file from the primeThemes.com server. It will download and extract the files to your current theme's functions folder. </p>
                <p>We recommend backing up your theme files before updating. Only upgrade the primeFramework if necessary.</p>
                <p>&rarr; <strong>Your version:</strong> <?php echo $local_version; ?></p>
                
                <p>&rarr; <strong>Current Version:</strong> <?php echo $remoteversion; ?></p>
                
                <input type="submit" class="button" value="Update Framework" />
                <?php } else { ?>                
                <h3>You have the latest version of primeFramework</h3>
                <p>&rarr; <strong>Your version:</strong> <?php echo $local_version; ?></p>
                <?php } ?>
                <input type="hidden" name="prime_update_save" value="save" />
                <input type="hidden" name="prime_ftp_cred" value="<?php echo base64_encode(serialize($_POST)); ?>" />
